...
describe("Key mapping", () => {
  test("changes keys, preserves data", () => {
    let inMap = Map.String.empty
      |. Map.String.set("a", 1)
      |. Map.String.set("b", 2)
      |. Map.String.set("c", 3);

    let outMap = inMap
      |. Holes.Map.String.mapKeys(Js.String.toUpperCase);

    let expectedMap = Map.String.empty
      |. Map.String.set("A", 1)
      |. Map.String.set("B", 2)
      |. Map.String.set("C", 3);
    ...
    /* TODO: custom matcher */
    /* TODO: refmt */
    expect(Map.String.eq(outMap, expectedMap, (==))) |>
      toBe(true);
  });
});
