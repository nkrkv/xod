...
template<> bool isInputDirty<input_GATE>(Context ctx) {
    return ctx->_isInputDirty_GATE;
}
...
template<> bool isInputDirty<input_PORT>(Context ctx) {
    return ctx->_isInputDirty_PORT;
}
...
template<> bool isInputDirty<input_SIG>(Context ctx) {
    return ctx->_isInputDirty_SIG;
}
...
void runTransaction(bool firstRun) {
    ...
    { // xod__core__digital_input #4
        if (node_4.isNodeDirty) {
            ...
            ctxObj._isInputDirty_PORT = firstRun;
    ...
    { // xod__core__gate #6
        if (node_6.isNodeDirty) {
            ...
            ctxObj._isInputDirty_GATE = node_4.isOutputDirty_SIG;
    ...
    { // xod__core__gate #7
        if (node_7.isNodeDirty) {
            ...
            ctxObj._isInputDirty_GATE = node_5.isOutputDirty_SIG;
            ctxObj._isInputDirty_TRIG = node_3.isOutputDirty_TICK;
    ...
    { // xod__core__digital_output #9
        if (node_9.isNodeDirty) {
            ...
            ctxObj._isInputDirty_PORT = firstRun;
            ctxObj._isInputDirty_SIG = node_8.isOutputDirty_MEM;
